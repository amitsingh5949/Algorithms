...
public class _937_ReorderDatainLogFiles {
	...
	public String[] reorderLogFiles1(String[] logs) {
		Arrays.parallelSort(logs, (s1,s2) -> {
			...
			if(Character.isDigit(s1.charAt(i1+1)) && Character.isDigit(s2.charAt(i2+1))) {
				...
				int x =  s1.substring(i1+1).compareTo(s2.substring(i2+1));
				if(x ==0) {
	...
	// same comparator doesn't work in above two methods, with TreeSet/TreeMap it works
