...
public class GameEventPattern {
	...
	public static List<String> getEventsOrder(String team1, String team2, List<String> event1, List<String> event2){
		List<List<String>> eventsTeam = new ArrayList<>();

		String pattern = "([a-zA-Z\\s]*)(\\d+)[+]?(\\d*).([G,Y,R,S])([a-zA-Z\\s]*)\"";
		...
		for(String temp : event1) {
			...
			record.add(m.group(4));
			record.add(m.group(5));
			eventsTeam.add(record);
		}
