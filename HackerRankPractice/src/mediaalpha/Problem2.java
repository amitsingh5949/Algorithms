...
public class Problem2 {
    ...
    public static void main(String[] args) {
       // System.out.println(new Parser("(( a + b ) + (( c + d )))").parse());
        ...
        //System.out.println(new Parser("1*(2+(3*(4+5)))").parse());
        //System.out.println(new Parser("2 + (3 / -5)").parse());
        System.out.println(new Parser("x+(y+z)+(t+(v+w))").parse());
...
class Parser {
    ...
    private Result factor() throws IllegalArgumentException {
        ...
        if(currToken() == '(') {
            result = paren();
        } else if(Character.isLetter(currToken()) ||Character.isDigit(currToken())) {
            result = variable();
        } else {
